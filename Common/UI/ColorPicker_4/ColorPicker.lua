...
function ColorPicker:Constructor(color, dimension)
    ...
    if (color) then
        color = LovelyMouseFinder.Utils.Color(color.R, color.G, color.B);
    else
        color = LovelyMouseFinder.Utils.Color(0.5, 1.0, 1.0);
    end
    ...
    self.color = LovelyMouseFinder.Utils.Color(1, 0.5, 0.5, 0.5);
    self.originalColor = LovelyMouseFinder.Utils.Color(color.R, color.G, color.B);
    ...
    self.radioButtons["G"] = LovelyMouseFinder.UI.RadioButton(self.radioContainer, L:GetText("Green"), false);
    ...
    self.radioButtons["B"] = LovelyMouseFinder.UI.RadioButton(self.radioContainer, L:GetText("Blue"), false);
    ...
    self.radioButtons["H"] = LovelyMouseFinder.UI.RadioButton(self.radioContainer, L:GetText("Hue"), false);
    ...
    self.radioButtons["S"] = LovelyMouseFinder.UI.RadioButton(self.radioContainer, L:GetText("Saturation"), false);
    ...
    self.radioButtons["V"] = LovelyMouseFinder.UI.RadioButton(self.radioContainer, L:GetText("Value"), false);
    ...
    for colorName, color in pairs(Turbine.UI.Color) do
        if (Turbine.UI.Color:IsA(color) and (color.A == 1)) then
            color = LovelyMouseFinder.Utils.Color(color.A, color.R, color.G, color.B);
...
function ColorPicker:AddRecentColor(newColor)
    ...
    for r = 1, #recentColors, 1 do
        ...
        oldColor = LovelyMouseFinder.Utils.Color(oldColor.A, oldColor.R, oldColor.G, oldColor.B);
...
function ColorPicker:HexChanged()
    ...
    if (R and G and B) then
        self.hex:SetBackColor(LovelyMouseFinder.Utils.Color(1, 1, 1, 1));
        ...
        self.hex:SetBackColor(LovelyMouseFinder.Utils.Color(1, 1, 0, 0));
    end
end

function ColorPicker:SetColor(color)
    self.suppressCallbacks = true;
...
function ColorPicker:SwatchClicked(color)
    self:SetColor(color);
    self:_ColorChanged();
end
...
function ColorPicker:FreezeDimension(dim, value)
    self.frozen = self.frozen or {};
    self.frozen[dim] = value;
    for _, d in pairs({ "R", "G", "B", "H", "S", "V" }) do
        if (dim ~= d) then
            self.radioButtons[d]:SetEnabled(false);
        end
    end
    self:SelectDimension(dim);
    self.slider:SetLocked(true);
    self:_ColorChanged();
end

function ColorPicker:_ColorChanged()
    local R, G, B = self.color.R, self.color.G, self.color.B;
    if (self.frozen) then
        for dim, value in pairs(self.frozen) do
            self.color:Set(dim, value);
        end
        if ((self.color.R ~= R) or (self.color.G ~= G) or (self.color.B ~= B)) then
            self:SetColor(self.color);
        end
    end
    if (not self.suppressCallbacks) then
