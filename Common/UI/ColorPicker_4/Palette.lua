...
function Palette:SetGamut(color)
    local sliderDimension = LovelyMouseFinder.Utils.Color.GetOtherDimensions(self.dimensionY);
    local sliderValue = color:Get(sliderDimension);
    if (string.find("RGB", sliderDimension)) then
        local backColor = LovelyMouseFinder.Utils.Color(sliderValue, 0, 0, 0);
        backColor[sliderDimension] = 1;
        self.overlay:SetBackColor(backColor);
    elseif (sliderDimension == "S") then
        local backColor = LovelyMouseFinder.Utils.Color(1 - sliderValue, 1, 1, 1);
        self.overlay:SetBackColor(backColor);
    elseif (sliderDimension == "H") then
        local backColor = LovelyMouseFinder.Utils.Color(1, 0, 0, 0);
        backColor:SetHSV(sliderValue, 1, 1);
        self:SetBackColor(backColor);
    else -- (sliderDimension == "V")
        local backColor = LovelyMouseFinder.Utils.Color(1 - sliderValue, 0, 0, 0);
        self.overlay:SetBackColor(backColor);
    end
end
...
function Palette:SetDimensions(dimensionX, dimensionY)
    ...
    local sliderDimension = LovelyMouseFinder.Utils.Color.GetOtherDimensions(dimensionY);
    if (string.find("RGB", sliderDimension)) then
