...
function Slider:SetGamut(color)
    if (string.find("RGB", self.dimension)) then
        local backColor = LovelyMouseFinder.Utils.Color(1, color.R, color.G, color.B);
        backColor[self.dimension] = 0;
        self.overlay:SetBackColor(backColor);
    elseif (self.dimension == "H") then
        local H, S, V = color:GetHSV();
        self:SetBackColor(LovelyMouseFinder.Utils.Color(1 - S, 1, 1, 1));
        self.overlay:SetBackColor(LovelyMouseFinder.Utils.Color(1 - V, 0, 0, 0));
